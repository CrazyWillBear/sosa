...
By default, the agent can only run commands from a pre-approved allowlist (`_ALLOWED` in `sosa/tools/Bash.py`). Any command containing a program not on that list is **denied automatically** unless you pass an `approval_fn`.
